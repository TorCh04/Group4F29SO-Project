...
function generateRandomData(currentTemperature, currentHumidity) {
  let humidity;
  let temperature;
  if (currentTemperature === undefined || currentTemperature === "Loading...") {
    temperature = 20;
  } else {
    temperature = parseInt(currentTemperature, 10);
  }

  if (currentHumidity === undefined || currentHumidity === "Loading...") {
    humidity = 50;
  } else {
    humidity = parseInt(currentHumidity, 10);
  }
  ...
  if (Math.random() < 0.33) {
    ...
    humidity = Math.max(
      47,
      Math.min(75, humidity + humidityChange * (Math.random() < 0.5 ? 1 : 2))
    ); // Ensure within range 47-75
  }
