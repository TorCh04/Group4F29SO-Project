...
// // get energy data for each day in a week
// router.get("/energyData/week", verifyToken, async (req, res) => {
//   try {
//     const user = await User.findById(req.user.id);
//     const devices = await Device.find({ _id: { $in: user.devices } }); // Find all devices that belong to the user
//     const energyData = Array(7).fill(0); // Array to store energy data for all devices
//     for (const device of devices) {
//         const energyUsage = await EnergyUsage.findOne({
//             device: device._id,
//         }).sort({ "data.timestamp": -1 }); // Find the latest energy data for the device
...
//             const dayDiff = Math.floor((new Date(data.timestamp).setHours(0, 0, 0, 0) - past7Days) / (1000 * 60 * 60 * 24));
//             if (dayDiff >= 0 && dayDiff < 7) {
//                 energyData[dayDiff] += data.energyUsage;
//             }
//         }
//     }
//     res.json(energyData);
